...
for this example, let's start small. instead of making a rollup with complicated logic, let's just post a blob with a single number in it for state. we'll call this number our "state". our rollup (you, in this case) will keep track of this number and update it every time we post a new blob.
